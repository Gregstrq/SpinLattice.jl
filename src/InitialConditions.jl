...
function random_q_state(Dh::Int64, RNG::AbstractRNG = Random.GLOBAL_RNG)
    threshold = (2.0^-7)::Float64
    q_state = Vector{Complex{Float64}}(undef, Dh)
...
function random_cl_state(cl_num, RNG::AbstractRNG = Random.GLOBAL_RNG)
...
function randomState(A::Approx, RNG::AbstractRNG = Random.GLOBAL_RNG) where {Approx<:ExactApprox}
    Dh = 2^A.L.tot_spin_num
    return random_q_state(Dh, RNG)
end
function randomState(A::ExactApprox, OSET::ObservablesSet, RNG::AbstractRNG = Random.GLOBAL_RNG)
    Dh = get_Dh(A)
...
function randomState(A::Approx, RNG::AbstractRNG = Random.GLOBAL_RNG) where {Approx<:ClusteredApprox}
    Dh = 2^(A.cluster_cell_num*A.L.cell_size)
    return VectorOfArray([random_q_state(Dh, RNG) for i in 1:A.cluster_num])
end
function randomState(A::Approx, RNG::AbstractRNG = Random.GLOBAL_RNG) where {Approx<:PureClassicalApprox}
    return random_cl_state(A.L.tot_spin_num, RNG)
end
function randomState(A::Approx, RNG::AbstractRNG = Random.GLOBAL_RNG) where {Approx<:HybridApprox}
    Dh = 2^A.q_num
    return ArrayPartition(random_q_state(Dh, RNG), random_cl_state(A.cl_num, RNG))
end
function randomState(A::CompositeApproximation{T}, RNG::AbstractRNG = Random.GLOBAL_RNG) where T<:HybridApprox
    Dh = 2^A.A1.q_num
    return ArrayPartition(random_q_state(Dh, RNG), random_cl_state(A.A1.cl_num, RNG), randomState(A.A2, RNG))
end

@inline randomState(A::abstractApproximation, OSET::ObservablesSet, RNG::AbstractRNG = Random.GLOBAL_RNG) = randomState(A, RNG)

function initRNG(d::Int)
    return randjump(Random.GLOBAL_RNG, d)
end
